...
type PlaneWreck = { root: THREE.Group; velocity: THREE.Vector3; rollVelocity: number; state: 'fall' | 'rest'; smokeAt: number; restUntil: number; team: Team; playerOwned: boolean; collider?: Collider }
...
type Grenade = { mesh: THREE.Mesh; velocity: THREE.Vector3; detonateAt: number; team: Team; playerOwned: boolean; owner?: Bot; smoke?: boolean; antiTank?: boolean; bomb?: boolean; mortar?: boolean; lastBounceAt?: number }
type ArtilleryShell = { position: THREE.Vector3; team: Team; impactAt: number; marker: THREE.Mesh }
type CannonShell = { mesh: THREE.Mesh; velocity: THREE.Vector3; life: number; team: Team; playerOwned: boolean; damage: number; penetration: number }
...
export class Game {
  ...
  private fixedMachineGuns() {
    this.campaign.flags.forEach((flag, index) => { const x = flag.x + (index % 2 ? -6 : 6), z = flag.z + (index % 3 - 1) * 3, y = this.terrainHeight(x, z), position = new THREE.Vector3(x, y, z), gun = this.createMachineGun(position, index % 2 ? Math.PI : 0, false); gun.root.position.y = y + 1; this.machineGuns.push(gun) })
  }
  ...
  touchMove(x: number, y: number) { this.keys[y < -.22 ? 'add' : 'delete']('KeyW'); this.keys[y > .22 ? 'add' : 'delete']('KeyS'); this.keys[x < -.25 ? 'add' : 'delete']('KeyA'); this.keys[x > .25 ? 'add' : 'delete']('KeyD'); this.keys[y < -.86 ? 'add' : 'delete']('ShiftLeft') }
  touchLook(dx: number, dy: number) { if (!this.active || this.dead) return; const scale = this.sensitivity * 1.7; if (this.playerPlane) this.planeStickPitch = THREE.MathUtils.clamp(this.planeStickPitch - dy * scale, -1, .95); else { this.yaw -= dx * scale; this.pitch = THREE.MathUtils.clamp(this.pitch - dy * scale, -1.35, 1.35) } }
  touchAction(action: 'fire' | 'aim' | 'reload' | 'grenade' | 'grenade-type' | 'smoke' | 'heal' | 'melee' | 'crouch' | 'prone' | 'interact' | 'switch' | 'jump' | 'brace', pressed = true) { if (!this.active || this.dead) return; if (action === 'fire') { this.firing = pressed; if (pressed && (!this.playerTransport || this.playerTransportPassenger)) this.playerTank ? this.firePlayerTank() : this.playerMortar ? this.fireMortar() : this.playerEmplacement ? this.fireEmplacement() : this.playerMG ? this.fireMachineGun() : this.fire(); return } if (action === 'grenade') { if (this.playerPlane) { if (pressed) this.dropPlaneBomb() } else if (pressed) this.beginGrenadeCook(); else this.releaseGrenadeCook(); return } if (action === 'aim' && this.playerTank) { this.tankMgFiring = pressed; if (pressed) this.firePlayerTankMg(); return } if (!pressed) return; if (action === 'aim' && this.playerPlane) this.dropPlaneBomb(); else if (action === 'aim' && (!this.playerTransport || this.playerTransportPassenger) && !this.playerTank && !this.playerMortar && !this.playerEmplacement && !this.playerMG) { this.aiming = !this.aiming; this.events.aim(this.aiming) } else if (action === 'reload') this.reload(); else if (action === 'grenade-type') this.cycleGrenade(); else if (action === 'smoke') this.throwSmoke(); else if (action === 'heal') this.heal(); else if (action === 'melee') this.melee(); else if (action === 'crouch' && this.playerTank) this.tankGunnerView = !this.tankGunnerView; else if (action === 'crouch' && !this.jumpHeight) this.stance = this.stance === 'crouch' ? 'stand' : 'crouch'; else if (action === 'prone' && !this.jumpHeight) this.stance = this.stance === 'prone' ? 'stand' : 'prone'; else if (action === 'interact') this.interact(); else if (action === 'switch') this.switchWeapon(this.weaponSlot === 'primary' ? 'sidearm' : 'primary'); else if (action === 'jump') this.jump(); else if (action === 'brace') this.toggleBrace() }
  ...
  respawn(loadoutIndex = 0, spawnId = 'base') {
    ...
    const spawnX = safe?.position.x ?? 0, spawnZ = (safe?.position.z ?? 42) + 6; this.groundLevel = this.terrainHeight(spawnX, spawnZ); this.camera.position.set(spawnX, this.groundLevel + 1.72, spawnZ); this.pitch = -.04; this.recoil = 0
  ...
  private clearRubble(rubble: THREE.Mesh[]) { for (const mesh of rubble) { mesh.removeFromParent(); mesh.geometry.dispose(); const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material]; materials.forEach(material => material.dispose()) } rubble.length = 0 }
  ...
  private moveTankWithCollision(position: THREE.Vector3, dx: number, dz: number) {
    const tank = this.tanks.find(candidate => candidate.alive && candidate.root.position === position), now = performance.now() / 1000
    if (tank && now < tank.ramSlowUntil) { dx *= .58; dz *= .58 }
    const overlaps = (box: Collider, x: number, z: number) => x + 1.15 > box.minX && x - 1.15 < box.maxX && z + 1.8 > box.minZ && z - 1.8 < box.maxZ
    ...
    const obstacles = [...this.vehicleColliders, ...this.fortifications.filter(item => item.kind === 'hedgehog').flatMap(item => item.colliders)], blocked = (x: number, z: number) => obstacles.some(box => overlaps(box, x, z)), width = this.battlefieldHalfWidth() - 2, nextX = THREE.MathUtils.clamp(position.x + dx, -width, width), nextZ = THREE.MathUtils.clamp(position.z + dz, -58, 58)
    const before = position.clone(); if (!blocked(nextX, position.z)) position.x = nextX; if (!blocked(position.x, nextZ)) position.z = nextZ
  ...
  private resolveTankInteractions(tank: Tank, movement: THREE.Vector3) {
    ...
    if (tank.team === 'enemy' && moving && !this.dead && !this.playerTank && tank.root.position.distanceToSquared(this.camera.position) < 2.2 ** 2) this.hurtPlayer(320, tank.root.position)
    for (const other of this.tanks) {
      ...
      delta.setY(0).normalize().multiplyScalar((3.1 - distance) * .5); tank.root.position.add(delta); if (!other.playerDriven) other.root.position.addScaledVector(delta, -1)
    }
    if (!moving) return
    for (let index = this.fortifications.length - 1; index >= 0; index--) {
      ...
      this.fortifications.splice(index, 1); for (const mesh of wire.meshes) { const point = mesh.position.clone(); this.scene.remove(mesh); mesh.geometry.dispose(); const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material]; materials.forEach(material => material.dispose()); for (let spark = 0; spark < 2; spark++) this.particle(point, new THREE.Vector3((Math.random() - .5) * 2, .4 + Math.random(), (Math.random() - .5) * 2), 0x5d5a4d, .055, .45, 2.5) }
  ...
  private findBotPath(from: THREE.Vector2, target: THREE.Vector2) {
    const spacing = 2, halfWidth = this.battlefieldHalfWidth(), columns = Math.floor(halfWidth * 2 / spacing) + 1, rows = 60, count = columns * rows
  ...
  private moveBot(bot: Bot, dx: number, dz: number, speed: number, dt: number) {
    const target = new THREE.Vector2(bot.root.position.x + dx, bot.root.position.z + dz), desired = this.botNavDirection(bot, target, performance.now() / 1000); if (!desired.lengthSq()) return false; desired.normalize()
    ...
    if (this.moveWithCollision(bot.root.position, desired.x * step, desired.y * step, .34)) { bot.root.position.y = this.terrainHeight(bot.root.position.x, bot.root.position.z); return true }
    const tangent = new THREE.Vector2(-desired.y * bot.strafe, desired.x * bot.strafe)
    if (this.moveWithCollision(bot.root.position, tangent.x * step, tangent.y * step, .34)) { bot.root.position.y = this.terrainHeight(bot.root.position.x, bot.root.position.z); return true }
    bot.strafe *= -1; tangent.multiplyScalar(-1)
    const moved = this.moveWithCollision(bot.root.position, tangent.x * step, tangent.y * step, .34); bot.root.position.y = this.terrainHeight(bot.root.position.x, bot.root.position.z); return moved
  }
  private lights() {
  ...
  private switchWeapon(slot: 'primary' | 'sidearm') {
    ...
    this.braced = false; this.weaponSlot = slot; [this.mag, this.reserve] = slot === 'primary' ? this.primaryAmmo : this.sidearmAmmo; this.aiming = false; this.events.aim(false); this.buildWeapon(); this.events.loadout(this.activeLoadout()); this.events.ammo(this.mag, this.reserve, false); this.status = slot === 'primary' ? '主武器' : '副武器'; this.statusUntil = performance.now() / 1000 + 1
  }
  ...
  private damageTankCrew(tank: Tank, crew: TankCrew, damage: number, playerOwned: boolean, headshot = false) { if (!tank.alive || !crew.alive) return; crew.hp -= damage; if (crew.hp > 0) { if (playerOwned) this.events.hit(false, headshot); return } crew.alive = false; crew.respawnAt = performance.now() / 1000 + 9; if (crew.mesh) crew.mesh.visible = false; tank.crew = tank.crewMen.filter(member => member.alive).length; if (playerOwned && tank.team === 'enemy') { const roleName = crew.role === 'driver' ? '驾驶员' : crew.role === 'gunner' ? '炮手' : crew.role === 'loader' ? '装填手' : '车长'; this.events.hit(true, headshot); this.events.kill(++this.kills, `日军坦克${roleName}`, headshot) } }
  ...
  private emplacement(team: Team, kind: 'at' | 'aa', x: number, z: number) {
    const root = new THREE.Group(); root.position.set(x, this.terrainHeight(x, z), z); root.rotation.y = team === 'ally' ? Math.PI : 0; this.scene.add(root); const color = team === 'ally' ? 0x4c5d50 : 0x696344
  ...
  private input() {
    ...
    addEventListener('mousemove', e => { if (!this.active || document.pointerLockElement !== this.canvas || this.dead || performance.now() < this.pointerLockGraceUntil) return; const movementX = THREE.MathUtils.clamp(Number.isFinite(e.movementX) ? e.movementX : 0, -160, 160), movementY = THREE.MathUtils.clamp(Number.isFinite(e.movementY) ? e.movementY : 0, -160, 160), zoomScale = this.aiming ? this.camera.fov / 72 : 1; if (this.playerPlane) this.planeStickPitch = THREE.MathUtils.clamp(this.planeStickPitch - movementY * this.sensitivity * zoomScale, -1, .95); else { this.yaw -= movementX * this.sensitivity * zoomScale; this.pitch = THREE.MathUtils.clamp(this.pitch - movementY * this.sensitivity * zoomScale, -1.35, 1.35) } })
  ...
  private updateMachineGuns(dt: number, time: number) {
    ...
    for (const machineGun of this.machineGuns) {
      ...
      if (target.bot) { target.bot.hp -= 22; target.bot.suppression = Math.min(1.6, target.bot.suppression + .24); if (target.bot.hp <= 0) { operator.kills++; this.down(target.bot, undefined, false, operator) } } else if (target.player) { this.hurtPlayer(8 * this.enemyDamage, start); if (this.dead) operator.kills++ }
    }
  }
  private repairTank() {
  ...
  private leaveTank() {
    const tank = this.playerTank; if (!tank) return; tank.playerDriven = false; this.tankMgFiring = false; this.tankGunnerView = false; this.playerTank = undefined; this.weapon.visible = true; const side = new THREE.Vector3(2.4, 0, 0).applyQuaternion(tank.root.quaternion); this.camera.position.copy(tank.root.position).add(side); this.groundLevel = this.terrainHeight(this.camera.position.x, this.camera.position.z); this.camera.position.y = this.groundLevel + 1.72; this.events.vehicle(false, 0, false); this.events.tankSight(false, false, false); this.events.loadout(this.activeLoadout()); this.events.ammo(this.mag, this.reserve, false)
  }
  private enterTransport(transport: Transport) { this.dismountTransportDriver(transport); this.playerTransport = transport; this.playerTransportPassenger = false; this.playerTransportSeat = -1; transport.playerDriven = true; this.weapon.visible = false; this.aiming = false; this.yaw = transport.root.rotation.y; this.pitch = -.04; this.events.aim(false); this.events.vehicle(true, transport.hp, true, transport.name, '运输载具 · E 下车', transport.maxHp, 1) }
  private enterTransportPassenger(transport: Transport) { this.playerTransport = transport; this.playerTransportPassenger = true; this.playerTransportSeat = 5; this.weapon.visible = true; this.aiming = false; this.yaw = transport.root.rotation.y + Math.PI; this.pitch = -.04; this.events.aim(false); this.events.vehicle(false, 0, false); this.events.loadout({ ...this.activeLoadout(), weapon: `${this.activeLoadout().weapon} · 搭乘卡车` }); this.events.ammo(this.mag, this.reserve, false); this.status = '已登上车斗，可使用当前武器'; this.statusUntil = performance.now() / 1000 + 1.8 }
  private leaveTransport() { const transport = this.playerTransport; if (!transport) return; if (!this.playerTransportPassenger) transport.playerDriven = false; this.playerTransport = undefined; this.playerTransportPassenger = false; this.playerTransportSeat = -1; this.weapon.visible = true; const side = new THREE.Vector3(2.2, 0, 0).applyQuaternion(transport.root.quaternion); this.camera.position.copy(transport.root.position).add(side); this.groundLevel = this.terrainHeight(this.camera.position.x, this.camera.position.z); this.camera.position.y = this.groundLevel + 1.72; this.events.vehicle(false, 0, false); this.events.loadout(this.activeLoadout()); this.events.ammo(this.mag, this.reserve, false) }
  ...
  private leavePlane() { const plane = this.playerPlane; if (!plane) return; plane.playerDriven = false; plane.abandoned = true; plane.nextAttack = performance.now() / 1000; this.playerPlane = undefined; this.parachuting = true; this.weapon.visible = false; this.camera.position.copy(plane.root.position).add(new THREE.Vector3(0, -.8, 0)); this.events.vehicle(false, 0, false); this.deployParachute(); this.status = '降落伞已展开 · 无人战机正在坠落'; this.statusUntil = performance.now() / 1000 + 1.8 }
  ...
  private dropPlaneBomb() {
    ...
    this.status = `炸弹投放 · 剩余 ${plane.bombs}`; this.statusUntil = time + 1.3
  }
  ...
  private launchCannonShell(origin: THREE.Vector3, direction: THREE.Vector3, team: Team, playerOwned: boolean, damage: number) {
    ...
    mesh.position.copy(origin); this.scene.add(mesh); this.cannonShells.push({ mesh, velocity: direction.clone().normalize().multiplyScalar(82), life: 2.4, team, playerOwned, damage, penetration })
  }
  ...
  private impactCannonShell(shell: CannonShell, position: THREE.Vector3, hit?: THREE.Intersection<THREE.Object3D>) {
    ...
    if (tank && tank.team !== shell.team) this.damageTank(tank, shell.damage, shell.playerOwned, position, shell.penetration, shell.velocity)
  ...
  private updateCannonShells(dt: number) {
    for (let index = this.cannonShells.length - 1; index >= 0; index--) {
      ...
      if (!impact && shell.team === 'enemy' && !this.dead) { const closest = new THREE.Line3(start, start.clone().addScaledVector(direction, distance)).closestPointToPoint(this.camera.position, true, new THREE.Vector3()); if (closest.distanceTo(this.camera.position) < .65) { impact = closest; this.hurtPlayer(65, start) } }
  ...
  private firePlayerTankMg() {
    ...
    if (bot) { const headshot = hit?.object.userData.hitZone === 'head'; bot.hp -= headshot ? 30 : 18; const killed = bot.hp <= 0; this.events.hit(killed, headshot); if (killed) this.kill(bot, headshot) } else if (transport) this.damageTransport(transport, 2, true); else if (gun) this.damageEmplacement(gun, 2, true)
  }
  ...
  private updatePlayer(dt: number, time: number) {
    ...
    if (entangled && time >= this.nextWireDamage) { this.nextWireDamage = time + .8; this.hurtPlayer(4, this.camera.position); if (this.dead) return }
    this.updateBrace()
  ...
  private explode(grenade: Grenade) {
    ...
    for (const tank of this.tanks) { if (!tank.alive || tank.team === grenade.team) continue; const distance = tank.root.position.distanceTo(center); if (distance < vehicleRadius) { const wasAlive = tank.alive; this.damageTank(tank, (bomb ? 260 : mortar ? 85 : antiTank ? 390 : 48) * (1 - distance / vehicleRadius), grenade.playerOwned, center); if (wasAlive && !tank.alive && grenade.owner) { grenade.owner.kills++; this.events.combatKill(grenade.owner.name, grenade.owner.team, tank.name, tank.team) } } }
  ...
  private updateEquipment(time: number) {
    if (this.parachuting) { this.events.equipment(this.grenades, this.aid, this.medkitAvailable, this.buildCharges, this.buildLabel(), this.mortarAmmo, Boolean(this.mortar), 'WASD 控制伞降方向'); return }
    if (this.playerPlane) { this.events.equipment(this.grenades, this.aid, this.medkitAvailable, this.buildCharges, this.buildLabel(), this.mortarAmmo, Boolean(this.mortar), `左键机枪 · 右键/B 炸弹 ×${this.playerPlane.bombs} · W/S 油门 · E 跳伞`); return }
    if (this.playerTransport) { this.events.equipment(this.grenades, this.aid, this.medkitAvailable, this.buildCharges, this.buildLabel(), this.mortarAmmo, Boolean(this.mortar), this.playerTransportPassenger ? `车况 ${Math.ceil(this.playerTransport.hp)}/${this.playerTransport.maxHp} · 可射击、换弹、投掷 · 按 E 下车` : 'WASD 驾驶 · 按 E 下车部署'); return }
    ...
    if (this.playerEmplacement) { this.events.equipment(this.grenades, this.aid, this.medkitAvailable, this.buildCharges, this.buildLabel(), this.mortarAmmo, Boolean(this.mortar), '左键开火 · 鼠标瞄准 · 按 E 撤离炮位'); return }
  ...
  private playerDeathScene() {
    const token = ++this.deathSceneToken, startedAt = performance.now(), originY = this.camera.position.y, startPitch = this.pitch, roll = (Math.random() < .5 ? -1 : 1) * (.42 + Math.random() * .28)
    this.weapon.visible = false; this.events.downed(this.kills, this.deaths); document.exitPointerLock()
    const animate = () => {
      if (token !== this.deathSceneToken || !this.dead || this.matchOver) return
      const t = Math.min((performance.now() - startedAt) / 1550, 1), ease = 1 - (1 - t) ** 3
      this.camera.position.y = THREE.MathUtils.lerp(originY, .34, ease); this.camera.rotation.set(startPitch + ease * .32, this.yaw, roll * ease, 'YXZ')
      if (t < 1) requestAnimationFrame(animate); else this.events.death(this.kills, this.deaths)
    }
    animate()
  }
  ...
  private hurtPlayer(amount: number, source?: THREE.Vector3) {
    if (this.dead) return; this.interruptBandage(); this.clearBuildAction('建造被打断'); this.lastDamageAt = performance.now() / 1000; this.hp = Math.max(0, this.hp - Math.round(amount)); this.events.health(this.hp); this.audio.hurt(THREE.MathUtils.clamp(amount / 55, .35, 1))
    if (source) { const sourceAngle = Math.atan2(source.x - this.camera.position.x, source.z - this.camera.position.z), forwardAngle = this.yaw + Math.PI, relative = Math.atan2(Math.sin(sourceAngle - forwardAngle), Math.cos(sourceAngle - forwardAngle)); this.events.damageDirection(relative, THREE.MathUtils.clamp(amount / 65, .25, 1)) }
    ...
    if (!this.hp) { this.reloadToken++; this.loading = false; this.noteDeath(this.camera.position, 'ally'); this.clearGrenadeCook(); this.clearSupplyAction(); if (this.playerTank) { this.playerTank.playerDriven = false; this.playerTank = undefined; this.tankMgFiring = false; this.tankGunnerView = false; this.events.vehicle(false, 0, false); this.events.tankSight(false, false, false) } if (this.playerMortar) { this.playerMortar.occupied = false; this.playerMortar = undefined; this.mortarMarker.visible = false } if (this.playerMG) { this.playerMG.occupied = false; this.playerMG = undefined } this.dead = true; this.deaths++; this.tickets[0] = Math.max(0, this.tickets[0] - 1); this.playerDeathScene() }
  }
  ...
  private kill(bot: Bot, headshot = false, impulse?: THREE.Vector3) { this.down(bot, impulse, headshot); this.events.kill(++this.kills, '日军步兵', headshot); this.events.combatKill('你', 'ally', bot.name, bot.team, headshot) }
  ...
  private damageTank(tank: Tank, damage: number, playerOwned: boolean, impact?: THREE.Vector3, shellPen?: number, shotDirection?: THREE.Vector3) {
    if (!tank.alive) return
    tank.lastDamageAt = performance.now() / 1000
    const weapon = this.activeLoadout(), armorPen = shellPen ?? (playerOwned && weapon.vehicleDamage === damage ? weapon.armorPen : undefined), reportArmor = playerOwned || this.playerTank === tank
  ...
  private createPlaneWreck(plane: Plane, playerOwned: boolean) {
    ...
    this.planeWrecks.push({ root, velocity: forward, rollVelocity: (Math.random() < .5 ? -1 : 1) * (1.6 + Math.random() * 2.5), state: 'fall', smokeAt: 0, restUntil: 0, team: plane.team, playerOwned })
  }
  ...
  private bailOutPilot(plane: Plane) {
    ...
    if (root.position.distanceToSquared(this.camera.position) < 100 ** 2) { this.status = `${plane.name}飞行员跳伞`; this.statusUntil = performance.now() / 1000 + 2 }
  }
  private updateBailedPilots(dt: number, time: number) {
    for (let index = this.bailedPilots.length - 1; index >= 0; index--) {
      ...
      if (!pilot.landedAt) {
        ...
        pilot.root.position.y = pilot.bot ? 0 : .65; pilot.landedAt = time; if (pilot.bot) { pilot.chute.removeFromParent(); this.disposeGroup(pilot.chute); pilot.bot.chuting = false; pilot.bot.weapon.visible = true; pilot.bot.root.scale.set(1, 1, 1); pilot.bot.nextShot = time + 1.2; this.status = `${pilot.bot.name}跳伞落地，重新加入战斗`; this.statusUntil = time + 1.8 } else pilot.chute.visible = false
      }
  ...
  private damagePlane(plane: Plane, damage: number, playerOwned: boolean) {
    if (!plane.alive) return; plane.hp -= damage; if (plane.hp > 0 && plane.hp < plane.maxHp * .32) this.bailOutPilot(plane); const killed = plane.hp <= 0; if (playerOwned) this.events.hit(killed); if (!killed) return; if (plane.pilot) { const pilot = plane.pilot; plane.pilot = undefined; pilot.piloting = undefined; pilot.root.visible = true; pilot.body.layers.enable(0); pilot.head.layers.enable(0); pilot.weapon.visible = true; this.down(pilot) } this.createPlaneWreck(plane, playerOwned); plane.alive = false; plane.playerDriven = false; plane.root.visible = false; plane.respawnAt = performance.now() / 1000 + 30; this.tickets[plane.team === 'ally' ? 0 : 1] = Math.max(0, this.tickets[plane.team === 'ally' ? 0 : 1] - 4); this.explosionFx(plane.root.position.clone()); if (playerOwned && plane.team === 'enemy') this.events.kill(++this.kills, plane.name); if (this.playerPlane === plane) { this.playerPlane = undefined; this.weapon.visible = true; this.events.vehicle(false, 0, false); this.hurtPlayer(999) }
  }
  private damageStructure(structure: FieldStructure, damage: number) {
    ...
    for (let index = 0; index < 6; index++) { const rubble = this.box([.45 + Math.random() * .85, .22 + Math.random() * .38, .45 + Math.random() * .9], [(Math.random() - .5) * 3.6, .12 + Math.random() * .18, (Math.random() - .5) * 4], index % 3 ? 0x655a48 : 0x3f3a31, structure.root); rubble.rotation.set(Math.random() * .25, Math.random() * Math.PI, Math.random() * .25); structure.rubble.push(rubble) }
  ...
  private damageUrbanBuilding(building: UrbanBuilding, damage: number) {
    ...
    building.details.forEach(detail => detail.visible = false)
    building.body.position.y = .58; building.body.scale.y = .1; building.body.rotation.z = (Math.random() < .5 ? -1 : 1) * .035
    for (let index = 0; index < 9; index++) { const rubble = this.box([.65 + Math.random() * 1.4, .28 + Math.random() * .48, .65 + Math.random() * 1.5], [(Math.random() - .5) * 7, .15 + Math.random() * .25, (Math.random() - .5) * 8], index % 4 ? 0x5d554b : 0x373632, building.root); rubble.rotation.set(Math.random() * .3, Math.random() * Math.PI, Math.random() * .3); building.rubble.push(rubble) }
  ...
  private fireNpcTransportPassenger(bot: Bot, transport: Transport, weapon: ReturnType<Game['botWeapon']>, time: number) {
    ...
    if (targetBot) targetBot.suppression = Math.min(1.6, targetBot.suppression + weapon.suppression); if (!hit) return; const damage = THREE.MathUtils.lerp(weapon.damage[0], weapon.damage[1], Math.random()) * .85; if (targetingPlayer) { this.hurtPlayer(damage * .36 * this.enemyDamage, start); if (this.dead) bot.kills++ } else if (targetBot) { targetBot.hp -= damage; if (targetBot.hp <= 0) { bot.kills++; this.down(targetBot, undefined, false, bot) } }
  }
  ...
  private updateMedicRescue(bot: Bot, dt: number, time: number) {
    ...
    this.reviveBot(target, time, bot); bot.reviveTarget = undefined; bot.reviveUntil = 0; bot.nextAidAt = Math.max(bot.nextAidAt, time + 5); bot.nextShot = time + .7
    return true
  }
  private updateBots(dt: number, time: number) {
    ...
    for (const team of ['ally', 'enemy'] as const) {
      ...
      let reinforced = 0
      if (this.tickets[ticketIndex] > 0) for (const bot of this.bots) if (bot.team === team && !bot.alive && time >= bot.respawnAt) { this.reinforce(bot, time); reinforced++ }
      if (reinforced) arrivals.push(`${team === 'ally' ? '友军' : '敌军'} ${reinforced} 人`)
      this.reinforcementWaveAt[team] = time + 6
    }
    if (arrivals.length) { this.status = `增援抵达 · ${arrivals.join(' · ')}`; this.statusUntil = time + 2.2 }
    ...
    for (const bot of this.bots) {
      ...
      if (meleeTarget) {
        if (time >= bot.nextMelee) { bot.nextMelee = time + 1.05; bot.meleeAt = time; bot.nextShot = Math.max(bot.nextShot, time + .7); this.audio.click(980, this.soundAt(bot.root.position).volume * .12); if (targetBot) { targetBot.hp -= 68; if (targetBot.hp <= 0) { bot.kills++; this.down(targetBot, targetBot.root.position.clone().sub(bot.root.position), false, bot) } } else { this.hurtPlayer(42 * this.enemyDamage, bot.root.position); if (this.dead) bot.kills++ } }
        continue
      }
      if (bot.reloadUntil) {
      ...
      this.hurtPlayer(damage * .36 * this.enemyDamage, bot.root.position); if (this.dead) { bot.kills++; return }
    }
  }
  ...
  private updateTanks(dt: number, time: number) {
    ...
    for (const tank of this.tanks) {
      ...
      if (combatTarget) { tank.turret.rotation.y = worldYaw - tank.root.rotation.y; const targetHeight = targetingPlayer ? this.camera.position.y : targetTank || targetTransport ? targetPos.y + 1 : targetPos.y + 1.2, gunHeight = tank.root.position.y + 1.56; tank.pitch.rotation.x = THREE.MathUtils.damp(tank.pitch.rotation.x, THREE.MathUtils.clamp(Math.atan2(targetHeight - gunHeight, Math.max(1, distance)), -.18, .35), 4, dt); if ((targetBot || targetingPlayer) && distance < 52 && time >= tank.nextMg && time >= tank.mgPauseUntil) { if (!tank.mgBurst) tank.mgBurst = 5 + Math.floor(Math.random() * 5); tank.nextMg = time + .115; const start = tank.coaxMuzzle.getWorldPosition(new THREE.Vector3()), end = targetPos.clone().setY(targetingPlayer ? this.camera.position.y : 1.3); end.x += (Math.random() - .5) * distance * .022; end.y += (Math.random() - .5) * distance * .014; this.tracer(start, end, tank.team === 'ally' ? 0xffdd83 : 0xff8068); const sound = this.soundAt(tank.root.position); this.audio.shot('auto', sound.volume * .7, sound.pan); if (Math.random() < THREE.MathUtils.clamp(.78 - distance * .012, .18, .66)) { if (targetBot) { targetBot.hp -= 16; if (targetBot.hp <= 0) this.down(targetBot) } else this.hurtPlayer(7 * this.enemyDamage, start) } if (!--tank.mgBurst) tank.mgPauseUntil = time + .6 + Math.random() * .6 } if (distance < 58 && time >= tank.nextShot) { tank.nextShot = time + 4.3 + Math.random() * 1.8; const origin = tank.muzzle.getWorldPosition(new THREE.Vector3()), end = targetPos.clone().setY(targetHeight), accuracy = THREE.MathUtils.clamp(.85 - distance * .008, .42, .78); if (Math.random() > accuracy) { end.x += (Math.random() - .5) * 6; end.z += (Math.random() - .5) * 5 } this.launchCannonShell(origin, end.sub(origin).normalize(), tank.team, false, 175); const sound = this.soundAt(tank.root.position); this.audio.shot('bolt', sound.volume, sound.pan) } }
    }
  }
  ...
  private updatePlanes(dt: number, time: number) {
    for (const plane of this.planes) {
      ...
      const start = plane.root.position.clone(), hit = Math.random() < .48; if (!hit) { target.x += (Math.random() - .5) * 7; target.z += (Math.random() - .5) * 7 } for (let i = 0; i < 4; i++) setTimeout(() => this.tracer(start.clone().add(new THREE.Vector3(0, 0, -i * .35)), target.clone().add(new THREE.Vector3((Math.random() - .5) * 2, 0, (Math.random() - .5) * 2)), plane.team === 'ally' ? 0xffdf83 : 0xff8068), i * 70); const sound = this.soundAt(start); this.audio.shot('auto', sound.volume, sound.pan); if (!hit) continue; if (targetPlayer) this.hurtPlayer(22, start); else if (targetBot) { targetBot.hp -= 55; if (targetBot.hp <= 0) this.down(targetBot) }
    }
  }
  ...
  private update(dt: number, time: number) {
    ...
    if (!this.dead) { if (this.parachuting) this.updateParachute(dt); else if (this.playerPlane) this.updatePlayerPlane(dt); else if (this.playerTank) this.updatePlayerTank(dt); else if (this.playerTransportPassenger) this.updatePlayerTransportPassenger(); else if (this.playerTransport) this.updatePlayerTransport(dt); else if (this.playerMortar) this.updatePlayerMortar(dt); else if (this.playerEmplacement) this.updatePlayerEmplacement(); else if (this.playerMG) this.updatePlayerMachineGun(dt); else this.updatePlayer(dt, time) }

...
export class Game {
  ...
  private input() {
    ...
    addEventListener('mousemove', e => { if (!this.active || document.pointerLockElement !== this.canvas || performance.now() < this.pointerLockGraceUntil) return; const movementX = THREE.MathUtils.clamp(Number.isFinite(e.movementX) ? e.movementX : 0, -160, 160), movementY = THREE.MathUtils.clamp(Number.isFinite(e.movementY) ? e.movementY : 0, -160, 160), zoomScale = this.aiming ? this.camera.fov / 72 : 1; if (this.dead) { this.downedFreeLook = true; this.yaw -= movementX * this.sensitivity; this.pitch = THREE.MathUtils.clamp(this.pitch - movementY * this.sensitivity, -.85, .85); this.camera.rotation.set(this.pitch, this.yaw, 0, 'YXZ'); return } if (this.playerPlane) this.planeStickPitch = THREE.MathUtils.clamp(this.planeStickPitch - movementY * this.sensitivity * zoomScale, -1, .95); else { this.yaw -= movementX * this.sensitivity * zoomScale; this.pitch = THREE.MathUtils.clamp(this.pitch - movementY * this.sensitivity * zoomScale, -1.35, 1.35) } })
  ...
  private hurtPlayer(amount: number, source?: THREE.Vector3, cause = '枪弹伤害') {
    if (this.dead) return; const dealt = Math.min(this.hp, Math.max(0, Math.round(amount))), distance = source ? Math.round(source.distanceTo(this.camera.position)) : 0
    ...
    this.logCombat('player_damage', { cause, damage: dealt, hpBefore: this.hp, distance, vehicle: this.playerTank?.name ?? this.playerTransport?.name ?? this.playerPlane?.name ?? null, source: source ? [Number(source.x.toFixed(2)), Number(source.y.toFixed(2)), Number(source.z.toFixed(2))] : null })

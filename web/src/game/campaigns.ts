...
const compactCampaigns: readonly Campaign[] = [
  { id: 'songhu', year: 1937, name: '淞沪会战', location: '上海', battlefield: '八字桥开战', date: '1937年8月13日', objective: '守住八字桥，并在入夜前发动反击', mode: '行动 · 三阶段城市攻防', rule: 'assault', attacker: 'enemy', theme: 'delta', weather: '阴雨', tickets: 180, allies: 7, enemies: 12, flags: [{ id: 'A1', x: -8, z: -30, radius: 6 }, { id: 'A2', x: 9, z: -28, radius: 6 }, { id: 'B1', x: -6, z: -4, radius: 6 }, { id: 'B2', x: 8, z: 4, radius: 6 }, { id: 'C1', x: -12, z: -18, radius: 6 }, { id: 'C2', x: 13, z: -12, radius: 6 }], layout: { road: 'rail-side', blocks: 'street', density: 1.15 } },
...
const MAP_SCALE_X = 2
const MAP_SCALE_Z = 2.5

export const campaigns: readonly Campaign[] = compactCampaigns.map(campaign => ({
  ...campaign,
  bounds: {
    halfWidth: (campaign.bounds?.halfWidth ?? 28) * MAP_SCALE_X,
    halfDepth: (campaign.bounds?.halfDepth ?? 58) * MAP_SCALE_Z,
  },
  flags: campaign.flags.map(flag => ({ ...flag, x: flag.x * MAP_SCALE_X, z: flag.z * MAP_SCALE_Z })),
}))
